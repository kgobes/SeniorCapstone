...
class TTSServer:
    ...
    def speech_handler(self, goal):
        ...
        if not (goal.speech == ''):
	  # name = speech_recog.getSpeakerName()
	    if goal.speech != "Hi! My name is Lili. What is yours?":
	    	if not goal.speech.startswith("What would you like to do?"):
			try:
	    			name = testSpeakerRecog.predictSpeaker('./enrolledSpeakers/currentSpeaker.wav')
			except Exception as e:
				print("in except")
				name = " " 
		    	goal.speech = "Okay, " + name + " " + goal.speech
